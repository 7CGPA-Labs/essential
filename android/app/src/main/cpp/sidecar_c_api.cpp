...
void* sidecar_init(const char* intent_path,
                   ...
                   const char* db_path) {
    std::string intent   = intent_path   ? intent_path   : "";
    std::string embed    = embed_path    ? embed_path    : "";
    std::string reranker = reranker_path ? reranker_path : "";
    std::string lang     = lang_path     ? lang_path     : "";
    std::string db       = db_path       ? db_path       : "";

    auto* coordinator = new SidecarPipelineCoordinator(
        intent, embed, reranker, lang, db);
...
void sidecar_destroy(void* handle) {
    if (!handle) return;
    delete static_cast<SidecarPipelineCoordinator*>(handle);
}
